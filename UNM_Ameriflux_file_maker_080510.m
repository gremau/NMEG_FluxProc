...
%     pt1_in=cat(1,ones(1,b).*NaN,pt1_in); % Add an extra row on top as for some reason gf file starts at 1am.    
    ...
    [aq, b]=size(pt2_in);
    pt2_in=cat(1,ones(1,b).*NaN,pt2_in); % Add an extra row on top as for some reason gf file starts at 1am.
   ...
   if sitecode~=7 
    ...
    RE_f =pt2_in(1:stop,6);
    GPP_f=pt2_in(1:stop,7);
    RE_f2 =pt1_in(1:stop,10);
    GPP_f2=pt1_in(1:stop,6);
    VPD_f=gf_in(1:stop,18);
    VPD_f=VPD_f./10; % To get it into kPa
    NEE_f3=gf_in(1:stop,21);
    NEE_f2=pt1_in(1:stop,11);
    ...
    H_f(~isnan(HSdry_massman))=HSdry_massman(~isnan(HSdry_massman));     
    ...
    HL_wpl_massman(isnan(E_wpl_massman))=NaN;
        ...
        elseif sitecode ==7
        header1 = {'YEAR','DOY','HRMIN','DTIME','UST','TA','WD','WS','NEE','FC',...
            'SFC','H','SSA','LE','SLE','G1','TS_2cm','TS_5cm','TS_10cm','PRECIP','RH','PA','CO2',...
            'VPD','SWC_2cm','SWC_5cm','SWC_10cm','RNET','PAR','PAR_DIFF','PAR_out','Rg','Rg_DIFF','Rg_out',...
            'Rlong_in','Rlong_out','FH2O','H20','RE','GPP','APAR'}; 
        end
        ...
        elseif sitecode==7
        header2 = {'YEAR','DOY','HRMIN','DTIME','UST','TA','TA_flag','WD','WS','NEE','FC','FC_flag',...
            'SFC','H','H_flag','SSA','LE','LE_flag','SLE','G1','TS_5cm','TS_20cm','TS_50cm','PRECIP','RH','PA','CO2',...
            'VPD','VPD_flag','SWC_5cm','SWC_20cm','SWC_50cm','RNET','PAR','PAR_DIFF','PAR_out','Rg','Rg_flag','Rg_DIFF','Rg_out',...
            'Rlong_in','Rlong_out','FH2O','H20','RE','RE_flag','GPP','GPP_flag','APAR'};  
        end
        ...
        datamatrix4 = [year,intjday,(hour.*100)+minute,jdayout,u_star,Tair_f,TA_flag,wnd_dir_compass,wnd_spd,...
            dummy,NEE_f,NEE_flag,dummy,H_f,H_flag,dummy,LE_f,LE_flag,dummy,ground,...
         ...
         subplot(5,1,1)
         plot(fc_raw_massman_wpl,'r.'); hold on
         plot(NEE_f,'b-'); hold on; 
         ylim([-20 20]); ylabel('NEE')
         subplot(5,1,2)
         plot(HSdry_massman,'r.'); hold on
         plot(H_f,'b-');
         ylabel('H')
         subplot(5,1,3)
         plot(HL_wpl_massman,'r.'); hold on
         plot(LE_f,'b-'); hold on; 
         ylim([-100 500]); ylabel('LE')
         subplot(5,1,4)
         plot(ground)
         ylabel('G')
         subplot(5,1,5)
         plot(NR_tot)
         ylim([-100 1000]); ylabel('Net Rad')
         
         figure('Name','Paritioned Fluxes','NumberTitle','off');
         subplot(3,1,1)
         plot(fc_raw_massman_wpl,'r.'); hold on
         plot(NEE_f,'b-'); hold on; 
         plot(NEE_f2,'g-'); hold on;
         ylim([-20 20]); ylabel('NEE'); legend('Obs','New gf','Old gf')
         subplot(3,1,2)
         plot(GPP_f,'b-'); hold on; 
         plot(GPP_f2,'g-'); hold on;
         ylim([0 20]); ylabel('GPP')
         subplot(3,1,3)
         plot(RE_f,'b-'); hold on; 
         plot(RE_f2,'g-'); hold on;
         ylim([0 20]); ylabel('RE')
                  
         orient landscape
         outname=strcat(afnames(sitecode,:),'_Partitioned_fluxes_',years,'.pdf');
         print('-dpdf', outname)
         
         figure('Name','Cumulative Fluxes','NumberTitle','off');
         subplot(2,1,1)
         title(year(1)); hold on
         yy=fc_raw_massman_wpl.*0.0216;
         yy(isnan(yy))=0;
         yy=cumsum(yy);
         NEE_f(isnan(NEE_f))=0; NEE_f2(isnan(NEE_f2))=0;
         plot(yy,'r'); hold on
         plot(cumsum(NEE_f.*0.0216),'b-'); hold on; 
         plot(cumsum(NEE_f2.*0.0216),'g-'); hold on;
         ylabel('\Sigma NEE (gC)')
         legend('Obs','New gf','Old gf')
         
         subplot(2,1,2)
         GPP_f(isnan(GPP_f))=0; GPP_f2(isnan(GPP_f2))=0;
         RE_f(isnan(RE_f))=0; RE_f2(isnan(RE_f2))=0;
         plot(cumsum(GPP_f.*0.0216),'b-'); hold on; 
         plot(cumsum(GPP_f2.*0.0216),'g-'); hold on;
         plot(cumsum(RE_f.*0.0216),'b+'); hold on; 
         plot(cumsum(RE_f2.*0.0216),'g+'); hold on;        
         ylabel('\Sigma (gC)')
         legend('New GPP','Old GPP','New RE','Old RE','location','northwest')
         
         orient landscape
         outname=strcat(afnames(sitecode,:),'_Cumulative_fluxes_',years,'.pdf');
         print('-dpdf', outname)
         
         for i = 1:363
             start = ((i-1)*48)+1; stop=i*48;
             dNEE(i)=nanmean(NEE_f(start:stop));
             dH(i)=nanmean(HSdry_massman(start:stop));
             dLE(i)=nanmean(LE_f(start:stop));
             dG(i)=nanmean(ground(start:stop));
             dR(i)=nanmean(NR_tot(start:stop));
         end
         ...
         plot(dLE,'r'); hold on
         plot(dG,'g'); hold on
         legend('dH','dLE','dG'); ylabel('Energy out'); ylim([-50 300])
         subplot(2,1,2)
         plot(dR); hold on
         plot(dR-(dH+dLE+dG),'r');
         legend('dR','Closure'); ylabel('Energy in')
         
         figure('Name','Soil temp and moisture','NumberTitle','off');
         subplot(2,1,1)
         plot(Tsoil_1); hold on; plot(Tsoil_2,'r'); hold on; plot(Tsoil_3,'g'); hold on
         legend('Tsoil 1','Tsoil 2','Tsoil 3'); ylabel('Soil temperature')
         subplot(2,1,2)
         plot(SWC_1); hold on; plot(SWC_2,'r'); hold on; plot(SWC_3,'g'); hold on
         legend('SWC 1','SWC 2','SWC 3'); ylabel('Soil moisture')
         
         figure('Name','Radiation','NumberTitle','off');
         subplot(3,1,1)
         plot(Rg_f); hold on; plot(sw_outgoing,'r'); legend('SW in','SW out')
         subplot(3,1,2)
         plot(lw_incoming); hold on; plot(lw_outgoing,'r'); 
...
timeo={'Created: ',time_out};
    ...
    outfilename = strcat(outfolder,afnames(sitecode,:),'_',years,'_gapfilled_2.xls');
    xlswrite(outfilename,sname,'data','A1');
    xlswrite(outfilename,email,'data','A2');
    xlswrite(outfilename,timeo,'data','A3');
