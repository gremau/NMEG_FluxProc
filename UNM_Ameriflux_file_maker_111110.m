...
%         vwc=repmat(-0.0663,(size(x_tc)))-0.00636.*x_tc+0.0007.*(x_tc.*x_tc); % temperature corrected
        ...
        figure;
        subplot(3,1,1)
        plot(vwc22(:,1),'b'); hold on
        plot(vwc3(:,1),'g'); hold on
        plot(vwc4(:,1),'r'); hold on        
        subplot(3,1,2)
        plot(vwc22(:,20),'b'); hold on
        plot(vwc3(:,20),'g'); hold on 
        plot(vwc4(:,20),'r'); hold on        
        subplot(3,1,3)        
        plot(vwc22(:,2),'b'); hold on
        plot(vwc3(:,2),'g'); hold on
        plot(vwc4(:,2),'r'); hold on   
...
elseif sitecode == 3 % Juniper savannah
   ...
   elseif year(1)==2009 || year(1)==2010
        vwc=data(:,178:195);
        vwc(vwc>1)=NaN; vwc(vwc<0)=NaN;
        SWC_1=nanmean(cat(2,vwc(:,1),vwc(:,4),vwc(:,7),vwc(:,10),vwc(:,13),vwc(:,16))'); SWC_1=SWC_1';
        SWC_2=nanmean(cat(2,vwc(:,2),vwc(:,5),vwc(:,8),vwc(:,11),vwc(:,14),vwc(:,17))'); SWC_2=SWC_2';
        SWC_3=nanmean(cat(2,vwc(:,3),vwc(:,6),vwc(:,9),vwc(:,12),vwc(:,15),vwc(:,18))'); SWC_3=SWC_3';
