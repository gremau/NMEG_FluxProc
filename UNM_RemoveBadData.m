...
elseif sitecode == 2 %SLand   added TWH, 4 Nov 2011, modifed Jul 2012
    % find soil heat flux plate measurements
    SHF_idx = find( cellfun( @(x) ~isempty(x), ...
                             regexp( headertext, '(shf.*)|(hfp.*)' ) ) );
    if numel( SHF_idx ) < 0
        %error( 'could not find soil heat flux observations' );
    end
    soil_heat_flux = data( :, SHF_idx );
    SHF_labels = format_SHF_labels( headertext( SHF_idx ) );

elseif sitecode == 3 %JSav   added TWH, 7 May 2012
    SHF_idx = find( ~cellfun( @isempty, regexp( headertext, 'shf_Avg.*' ) ) );
    soil_heat_flux = data( :, SHF_idx - 1 );
    SHF_labels = format_SHF_labels( headertext( SHF_idx ) );

    Tsoil = repmat( -999.0, size( data, 1 ), 1 );
...
% fprintf( '-------------------\n');
% fprintf( 'header\tformatted\n');
% for i = 1:numel( SHF_idx )
%     fprintf( '%s\t%s\n', headertext{ SHF_idx( i ) }, SHF_labels{ i } );
% end
% fprintf( '-------------------\n');

% Juniper S heat flux plates need multiplying by calibration factors
if sitecode == 3 
    cal_factors = repmat( [ 32.27, 33.00, 31.60, 32.20 ], ...
                          size( soil_heat_flux, 1 ), 1 );
    soil_heat_flux = soil_heat_flux .* cal_factors;
