...
elseif sitecode == 6
    site = 'MCon';
    z_CSAT = 23.9; sep2 = 0.375; angle = 71.66; h_
    canopy = 16.56;
    timestamp_col = 'BS';
    bad_variance_col = 'BP';
elseif sitecode == 7
...
%filein = strcat('C:\transfer\Flux_Tower_Data_by_Site\',site,'\',filename); % assemble path to file
filein = strcat('C:\Research_Flux_Towers\Flux_Tower_Data_by_Site\',site,'\',filename); % assemble path to file
datarange = strcat('BW',num2str(first_row),':','FC',num2str(last_row)); % specify what portion of spreadsheet to read in
headerrange = strcat('BW2:FC2'); % specify portion of spreadsheet that is headers
...
datarange2 = strcat('J',num2str(first_row),':BU',num2str(last_row));
...
[year month day hour minute second] = datevec(timestamp) % ,'dd/mm/yyyy HH:MM:SS'); %break timestamp into usable data and time variables
...
for i = 1:nrows
    ...
    elseif sitecode == 7 & year ~= 2008 % TX freeman
        stop
...
%         k(3) = 0.999915365;
    ...
    elseif sitecode == 7 & year == 2008 % Using the same as the latter part of 2007 
            b0 = -0.007905583;
            b1 = 0.012986531;
            b2 = -0.000801434;
            k(1) = -0.012985432;
    ...
    elseif sitecode == 9 % TX_grassland
        ...
        k(3) = 0.999822687;
            ...
            elseif sitecode == 10 || 11 % pinyon juniper - girdled
%             if speed >= 5
%                 b0 = -0.01340991;
%                 b1 = -0.01848391;
%                 b2 =  0.01754080;
...
%             end   
   ...
   % flux_co2(i) = (flux_co2(i)./1000000)*44*1000; % back to mg
    ...
    hh=((1./1000)./44).*1000000;
    cov_Ts_Ux(i)=cov_Ts_Ux(i).*hh;
    cov_Ts_Uy(i)=cov_Ts_Uy(i).*hh;
    cov_Ts_Uz(i)=cov_Ts_Uz(i).*hh;
...
% DATAOUT = [jday,blank',umean,vmean,wmean,Ts_meanC,TD,wind_direction,wind_speed,rH,cov_Ux_Ux,cov_Uy_Uy,cov_Uz_Uz,blank',blank',...
...
%R=8.314472e15;
%Fc_raw=Fc_raw./(R.*(Ts_meanK./press_mean)./1000)



 DATAOUT = [jday,blank',umean,vmean,wmean,Ts_meanC,TD,wind_direction,wind_speed,rH,cov_Ux_Ux,cov_Uy_Uy,cov_Uz_Uz,blank',blank',...
     blank',blank',cov_Ts_Ux,cov_Ts_Uy,cov_Ts_Uz,USTAR',cov_co2_Ux,cov_co2_Uy,cov_co2_Uz,co2_mean_out,blank',blank',blank',blank',h2o_Avg_out,blank',...
     Fc_raw',flux_co2_massman',Fc_water_term',Fc_heat_term_massman',Fc_corr_massman_ourwpl',...
     flux_h2o',flux_h2o_massman',E_water_term',E_heat_term_massman',E_corr_massman',blank',...
     HSdry',HSwet',HSwetwet',HSdry_massman',...
     flux_HL',flux_HL_massman',flux_HL_wpl_massman',rhoa_out',rhov_out',rhoc_out'];
...
for i = 1:nrows
 ...
 xlswrite (fileout,TO_WRITE,'test',strcat('J',num2str(first_row)));
