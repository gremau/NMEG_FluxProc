...
%        end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Set site specific parameters from site config file %
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     siteConfig = strcat(char(sitecode), '_Header_Resolutions.csv');
...
%     resolutions = resolutions_reshaped';
    ...
    varIdx = 'agc_Avg'
    agc_Avg = data(:,strcmp(headertext, varIdx))

    varIdx = 'h2o_hmp_mean_Avg'
    h2o_hmp = data(:,strcmp(headertext, varIdx))
           
    varIdx = 'RH_10_Avg'
    rH = data(:,strcmp(headertext, varIdx))
         
    varIdx = 'Ts_Avg'
    Tair_TOA5 = data(:,strcmp(headertext, varIdx))
             
    varIdx = 'rain_Tot'
    precip = data(:,strcmp(headertext, varIdx))
       
    varIdx = 'press_Avg'
    atm_press = data(:,strcmp(headertext, varIdx))
    
    varIdx = 'par_faceup_Avg'
    Par_Avg = data(:,strcmp(headertext, varIdx))
    
    varIdx = 'AirTC_6p85_Avg'
    air_temp_hmp = data(:,strcmp(headertext, varIdx))

    varIdx = 'NetTot_Avg'
    NR_tot = data(:,strcmp(headertext, varIdx))
    
    varIdx = 'Rad_short_Up_Avg'
    sw_incoming = data(:,strcmp(headertext, varIdx))
    
    varIdx = 'Rad_short_Dn_Avg'
    sw_outgoing = data(:,strcmp(headertext, varIdx))
    
    varIdx = 'CG3UpCo_Avg'
    lw_incoming = data(:,strcmp(headertext, varIdx))
    
    varIdx = 'CG3DnCo_Avg'
    lw_outgoing = data(:,strcmp(headertext, varIdx))

% PJ girdle, calculate relative humidity from hmp obs using helper
% function. Not sure it is needed.
   if sitecode == 10 & year == 2014
       rH2 = thmp_and_h2ohmp_2_rhhmp( air_temp_hmp, h2o_hmp ) ./ 100.0;
       replace_rH = find(rH == 0);
       rH(replace_rH) = rH2(replace_rH);
   end
   
   % Correct any rH values that happen to not be scaled appropriately
   rH(rH > 1) = rH(rH > 1) ./ 100;
    ...
    % remove HS data when wind direction removed
    % other flux variables, use existing wind direction variable
...
if (sitecode == 10 ) & ( year == 2014 )
    % days 1 to 40.5 -- low [CO2] but fluxes look ok
    co2_conc_filter_exceptions( DOYidx( 112.4 ) : DOYidx( 129.5 ) ) = true;
end
