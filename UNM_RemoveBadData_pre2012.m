...
RBDrc.LH_maxmin_flag = ( HL_wpl_massman > RBDrc.LH_max ) | ( HL_wpl_massman < RBDrc.LH_min );
LH_night_flag = ( LH_rad < 20.0 ) & ( abs( HL_wpl_massman ) > 20.0 );
LH_day_flag = ( LH_rad >= 20.0 ) & ( HL_wpl_massman < 0.0 );
if args.Results.draw_plots
    script_LE_diagnostic_plot;
end
removed_LH_wpl_mass = numel( find( RBDrc.LH_maxmin_flag | ...
                                   LH_night_flag | ...
                                   LH_day_flag ) );
HL_wpl_massman( RBDrc.LH_maxmin_flag | LH_night_flag | LH_day_flag ) = NaN;
% QC for sw_incoming
...
elseif args.Results.sitecode == 7
    ...
    nrow = numel( decimal_day );
    canopy_5cm = repmat( -999, nrow, 1 );
    canopy_10cm = repmat( -999, nrow, 1 );
    open_5cm = repmat( -999, nrow, 1 );
    open_10cm = repmat( -999, nrow, 1 );
    psoil_heat_flux_open = repmat( -999, nrow, 1 );
    soil_heat_flux_mescan = repmat( -999, nrow, 1 );
    soil_heat_flux_juncan = repmat( -999, nrow, 1 );
    soil_heat_flux_open = repmat( -999, nrow, 1 );
...
figure();
plot( decimal_day, out_data.Par_Avg, '.' )
title( sprintf( '%s %d', ...
                char( args.Results.sitecode ), ...
                args.Results.year ) );
ylabel( 'PAR' );
