...
% UNM_WRITE_FOR_GAPFILLER_FILE - given the necessary observed variables, writes
%   a data file in the format expected by Max Planck Institute's eddy covariance
%   gapfiller/flux partitioner.  The file generated by be passed directly to
%   UNM_run_gapfiller.
%
...
[ Year, ~, ~, Hour, Minute, ~ ] = datevec( args.Results.timestamp );
% calculate day of year
DOY = floor( args.Results.timestamp - datenum( Year, 1, 0 ) );
% specify times as fractional hours, not hours and minutes 
MINUTES_PER_HOUR = 60.0;
Hour = Hour + ( Minute / MINUTES_PER_HOUR );

if isempty( args.Results.VPD )
    VPD = repmat( NaN, size( args.Results.timestamp ) );
end

var_names = { 'Year', 'DoY', 'Hour', 'NEE', 'LE', 'H', 'Rg', 'Tair', ...
              'Tsoil', 'rH', 'VPD', 'Ustar' };
var_units = { '--', '--', '--', 'umolm-2s-1', 'Wm-2', 'Wm-2', 'Wm-2', ...
              'degC', 'degC', '%', 'hPa', 'ms-1' };
fgf = dataset( { [ Year, ...
                   DOY, ...
                   Hour, ...
