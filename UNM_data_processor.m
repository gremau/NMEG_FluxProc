...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if lag == 0
        [CO2_,H2O_,FCO2_,FH2O_,HSENSIBLE_,HLATENT_,RHOM_,TDRY_,IOKNUM_,zoL,UVWTVAR_,COVUVWT_,HBUOYANT_,USTAR_,TRANSPORT_,...
             u_vector_,w_mean_,rH_] = UNM_flux_031010(year_ts,month_ts,UVW2,uvwmean,SONDIAG,CO2',H2O',TD',RHO',IRGADIAG',rotation,site,sitecode,num,PWATER,uvwmeanrot(i,:),IRGAP,speed(i),temp2,theta(i));
             
        elseif lag==1
        [CO2_,H2O_,FCO2_,FH2O_,HSENSIBLE_,HLATENT_,RHOM_,TDRY_,IOKNUM_, LAGCO2, LAGH2O,zoL] = flux7500freeman_lag(UVW2,uvwmean,USTAR_,SONDIAG,CO2',H2O',TD',RHO',IRGADIAG',rotation, sitecode,num, PWATER,uvwmeanrot(i,1:3),hsout,IRGAP,theta(i));
        lagCO2(i,1:7)=LAGCO2;
        lagH2O(i,1:5)=LAGH2O;
        end
