...
PPine_07 = dlmread(filename,'',5,0);
filename = strcat(afnames(sitecode,:),'_2008_gapfilled.txt');
PPine_08 = dlmread(filename,'',5,0);
filename = strcat(afnames(sitecode,:),'_2009_gapfilled.txt');
PPine_09 = dlmread(filename,'',5,0);
filename = strcat(afnames(sitecode,:),'_2010_gapfilled.txt');
PPine_10 = dlmread(filename,'',5,0);



PPine_07(PPine_07==-9999)=nan; PPine_08(PPine_08==-9999)=nan; PPine_09(PPine_09==-9999)=nan;PPine_10(PPine_10==-9999)=nan;

cum_nee(:,1)=PPine_07(1:17416,11);
cum_nee(:,2)=PPine_08(1:17416,11);
cum_nee(:,3)=PPine_09(1:17416,11);
cum_nee(:,4)=PPine_10(1:17416,11);
...
for i =1:ndays
   daily_values(i,1)=sum(PPine_07(PPine_07(:,2)==i,11).*0.0216);
   daily_values(i,2)=sum(PPine_08(PPine_08(:,2)==i,11).*0.0216); 
   daily_values(i,3)=sum(PPine_09(PPine_09(:,2)==i,11).*0.0216); 
   daily_values(i,4)=sum(PPine_10(PPine_10(:,2)==i,11).*0.0216); 
end
...
legend('2007','2008','2009','2010'); hold on
