...
% site.
%
%Input Description (either number or vectors):
% UTC (Coordinated Universal Time serial_date_number)
...
%
%Modified by RJL 03/28/2014 to allow vector math.

% Code Sequence

% Compute JD. Serial date number output.
jd = juliandate( UTC );
d = jd - 2451543.5;
...
jd = floor(365.25 .* (year + 4716.0)) + floor(30.6001 .* (month + 1.0)) + ...
     2.0 - floor(year ./ 100.0) + floor(floor(year ./ 100.0 ) ./ 4.0 ) + ...
     day - 1524.5 + (hour + min ./ 60 + sec ./ 3600) ./ 24;
