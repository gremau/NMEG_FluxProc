function [CO2OUT,H2OOUT,FCO2,FH2O,HSENSIBLE,HLATENT,RHOM,TDRY,OKNUM,zoL,...
          UVWTVAR,COVUVWT,HBUOYANT,USTAR,TRANSPORT,u_vector,w_mean,rH]=...
          UNM_flux(year_ts,month_ts,UVW2,uvwmean,SONDIAG,CO2,H2O,TD,RHO,...
          irgadiag,rotation,site,sitecode,num,PWATER,uvwmeanrot,IRGAP,...
          speed,temp2,theta)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% create a vector of datenums from data timestamps
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
daten = datenum(year_ts, month_ts, day_ts);
...
elseif sitecode == 10 % here for PJ_girdle
  z_CSAT = repmat(5.5, 1, length(date_n))
...
if length(iok) < 6000  %length(find(iok))<6000
    ...
    Lv = mean( (2.501*ones(size(iok))-0.00237*(TDlag(2,iok)-273.15))*10^3); % (J/Kg) from Stull p 641
    ...
    % Fc_raw = (Fc_raw.*1.1069)-0.2716;
    
    [Uz_co2_c,Uz_h2o_c,Uz_Ts_c,Fc_c,LE_c,Hs_wet_c,Hs_dry_c,H_wet_c,James_water_term,James_heat_term,zoL,Uz_rhov_c] = UNM_WPLMassman(uvwmean,wTdmax2,E_raw*0.018,Fc_raw*0.044,MEANCO2,MEANTD,MEANRHO,USTAR,hsout,sep2,angle,z_CSAT,IRGAP*1000,MEANPWATER,Lv,h_canopy,wrhovmax2*0.018);
