function [CO2OUT, H2OOUT, FCO2, FH2O, HSENSIBLE, HLATENT, RHOM, TDRY, OKNUM, ...
	  zoL, UVWTVAR, COVUVWT, HBUOYANT, USTAR, TRANSPORT, u_vector, w_mean, ...
	  rH] =  UNM_flux(year_ts, month_ts, day_ts, UVW2, uvwmean, ...
...
if length(iok) < 6000  %length(find(iok))<6000
    ...
    if strcmp(rotation, '3d'); %3D rotation-- keep variables the same
        ...
        vhl2max2=vhl2max;
        whl2max2=whl2max;
    elseif strcmp(rotation, 'planar');  %planar rotation: determine scalar flux in new
                         %coordinate (code from HANDBOOK OF MICROMETEOROLOGY
                         %P. 63)
...
%     end
    ...
    if strcmp(rotation, '3d'); %3D rotation-- keep variables the same
        ...
        vco2max2=vco2max;
        wco2max2=wco2max;
    elseif strcmp(rotation, 'planar');  %planar rotation: determine scalar flux in new coordinate (code from HANDBOOK OF MICROMETEOROLOGY P. 63)
